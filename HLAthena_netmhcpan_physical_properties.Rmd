...
colnames(human_new_filt)[1] <- "Peptide"
...
write.table(kmers8, "8mers_all_sources.csv", row.names = FALSE, quote = FALSE, col.names=FALSE, sep=",")
write.table(kmers9, "9mers_all_sources.csv", row.names = FALSE, quote = FALSE, col.names = FALSE, sep=",")
write.table(kmers10, "10mers_all_sources.csv", row.names = FALSE, quote = FALSE, col.names =FALSE, sep=",")
write.table(kmers11, "11mers_all_sources.csv", row.names = FALSE, quote = FALSE, col.names =FALSE, sep=",")

##awk -F, '{print $1}' 8mers_all_sources.csv > 8mers_peps.csv
##awk -F, '{print $1}' 9mers_all_sources.csv > 9mers_peps.csv
##awk -F, '{print $1}' 10mers_all_sources.csv > 10mers_peps.csv
##awk -F, '{print $1}' 10mers_all_sources.csv > 11mers_peps.csv
##python3 initial_umap.py 8mers_peps.csv 8mers_properties.csv
##python3 initial_umap.py 9mers_peps.csv 9mers_properties.csv
##python3 initial_umap.py 10mers_peps.csv 10mers_properties.csv
##python3 initial_umap.py 11mers_peps.csv 11mers_properties.csv

```

```{r fix HU kmers}
kmers8 <- read.csv("/Users/nguyenau/Documents/Coding/PeptideAlleleBinding/8mers_all_sources.csv", header=FALSE)
kmers8 <- tibble(kmer8)
kmers8 <- kmers8 %>% add_row(V1=human_new_filt[nchar(human_new_filt$Peptide) == 8,]$Peptide, V2="human", V3=human_new_filt[nchar(human_new_filt$Peptide) == 8,]$Score)
write.table(kmers8, "8mers_all_sources.csv", row.names = FALSE, quote = FALSE, col.names=FALSE, sep=",")
rm(kmers8)

kmers9 <- read.csv("/Users/nguyenau/Documents/Coding/PeptideAlleleBinding/9mers_all_sources.csv", header=FALSE)
kmers9 <- tibble(kmers9)
kmers9 <- kmers9 %>% add_row(V1=human_new_filt[nchar(human_new_filt$Peptide) == 9,]$Peptide, V2="human", V3=human_new_filt[nchar(human_new_filt$Peptide) == 9,]$Score)
write.table(kmers9, "9mers_all_sources.csv", row.names = FALSE, quote = FALSE, col.names = FALSE, sep=",")
rm(kmers9)

kmers10 <- read.csv("/Users/nguyenau/Documents/Coding/PeptideAlleleBinding/10mers_all_sources.csv", header=FALSE)
kmers10 <- tibble(kmers10)
kmers10 <- kmers10 %>% add_row(V1=human_new_filt[nchar(human_new_filt$Peptide) == 10,]$Peptide, V2="human", V3=human_new_filt[nchar(human_new_filt$Peptide) == 10,]$Score)
write.table(kmers10, "10mers_all_sources.csv", row.names = FALSE, quote = FALSE, col.names =FALSE, sep=",")
rm(kmers10)

kmers11 <- read.csv("/Users/nguyenau/Documents/Coding/PeptideAlleleBinding/11mers_all_sources.csv", header=FALSE)
kmers11 <- tibble(kmers11)
kmers11 <- kmers11 %>% add_row(V1=human_new_filt[nchar(human_new_filt$Peptide) == 11,]$Peptide, V2="human", V3=human_new_filt[nchar(human_new_filt$Peptide) == 11,]$Score)
write.table(kmers11, "11mers_all_sources.csv", row.names = FALSE, quote = FALSE, col.names =FALSE, sep=",")
rm(kmers11)
...
```{r merge files for 8mers}
map8_peps <- read.csv("8mer_pep_umap.csv", header=FALSE)
colnames(map8_peps) <- c("Peptide", "X", "Y")
merged_peps_umap8 <- merge(map8_peps, betaherpes_6a_new_filt, by="Peptide")
merged_peps_umap8$Source <- "betaherpes_6a"
temp <- merge(map8_peps, bk_new_filt, by="Peptide")
temp$Source <- "bk"
merged_peps_umap8 <- rbind(merged_peps_umap8, temp)
temp <- merge(map8_peps, cmv_new_filt, by="Peptide")
temp$Source <- "cmv"
merged_peps_umap8 <- rbind(merged_peps_umap8, temp)
temp <- merge(map8_peps, covid_new_filt, by="Peptide")
temp$Source <- "covid"
merged_peps_umap8 <- rbind(merged_peps_umap8, temp)
temp <- merge(map8_peps, ebv_new_filt, by="Peptide")
temp$Source <- "ebv"
merged_peps_umap8 <- rbind(merged_peps_umap8, temp)
temp <- merge(map8_peps, herpes_1_17_new_filt, by="Peptide")
temp$Source <- "herpes_1_17"
merged_peps_umap8 <- rbind(merged_peps_umap8, temp)
temp <- merge(map8_peps, herpes_2_hg52_new_filt, by="Peptide")
temp$Source <- "herpes_2_hg52"
merged_peps_umap8 <- rbind(merged_peps_umap8, temp)

colnames(human_new_filt)[1] <- "Peptide"
temp <- merge(map8_peps, human_new_filt, by="Peptide")
temp$Source <- "human"
merged_peps_umap8 <- rbind(merged_peps_umap8, temp)
temp <- merge(map8_peps, random_new_filt, by="Peptide")
temp$Source <- "random"
merged_peps_umap8 <- rbind(merged_peps_umap8, temp)

write.table(merged_peps_umap8, "merged_peps_umap8mer.csv", row.names=FALSE, quote=FALSE, sep=',')
```

```{r merge files for 9mers}
map9_peps <- read.csv("9mer_pep_umap.csv", header=FALSE)
colnames(map9_peps) <- c("Peptide", "X", "Y")
merged_peps_umap9 <- merge(map9_peps, betaherpes_6a_new_filt, by="Peptide")
merged_peps_umap9$Source <- "betaherpes_6a"
temp <- merge(map9_peps, bk_new_filt, by="Peptide")
temp$Source <- "bk"
merged_peps_umap9 <- rbind(merged_peps_umap9, temp)
temp <- merge(map9_peps, cmv_new_filt, by="Peptide")
temp$Source <- "cmv"
merged_peps_umap9 <- rbind(merged_peps_umap9, temp)
temp <- merge(map9_peps, covid_new_filt, by="Peptide")
temp$Source <- "covid"
merged_peps_umap9 <- rbind(merged_peps_umap9, temp)
temp <- merge(map9_peps, ebv_new_filt, by="Peptide")
temp$Source <- "ebv"
merged_peps_umap9 <- rbind(merged_peps_umap9, temp)
temp <- merge(map9_peps, herpes_1_17_new_filt, by="Peptide")
temp$Source <- "herpes_1_17"
merged_peps_umap9 <- rbind(merged_peps_umap9, temp)
temp <- merge(map9_peps, herpes_2_hg52_new_filt, by="Peptide")
temp$Source <- "herpes_2_hg52"
merged_peps_umap9 <- rbind(merged_peps_umap9, temp)
colnames(human_new_filt)[1] <- "Peptide"
temp <- merge(map9_peps, human_new_filt, by="Peptide")
temp$Source <- "human"
merged_peps_umap9 <- rbind(merged_peps_umap9, temp)
temp <- merge(map9_peps, random_new_filt, by="Peptide")
temp$Source <- "random"
merged_peps_umap9 <- rbind(merged_peps_umap9, temp)

write.table(merged_peps_umap9, "merged_peps_umap9mer.csv", row.names=FALSE, quote=FALSE, sep=',')
```

```{r merge files for 10mers}
map10_peps <- read.csv("10mer_pep_umap.csv", header=FALSE)
colnames(map10_peps) <- c("Peptide", "X", "Y")
merged_peps_umap10 <- merge(map10_peps, betaherpes_6a_new_filt, by="Peptide")
merged_peps_umap10$Source <- "betaherpes_6a"
temp <- merge(map10_peps, bk_new_filt, by="Peptide")
temp$Source <- "bk"
merged_peps_umap10 <- rbind(merged_peps_umap10, temp)
temp <- merge(map10_peps, cmv_new_filt, by="Peptide")
temp$Source <- "cmv"
merged_peps_umap10 <- rbind(merged_peps_umap10, temp)
temp <- merge(map10_peps, covid_new_filt, by="Peptide")
temp$Source <- "covid"
merged_peps_umap10 <- rbind(merged_peps_umap10, temp)
temp <- merge(map10_peps, ebv_new_filt, by="Peptide")
temp$Source <- "ebv"
merged_peps_umap10 <- rbind(merged_peps_umap10, temp)
temp <- merge(map10_peps, herpes_1_17_new_filt, by="Peptide")
temp$Source <- "herpes_1_17"
merged_peps_umap10 <- rbind(merged_peps_umap10, temp)
temp <- merge(map10_peps, herpes_2_hg52_new_filt, by="Peptide")
temp$Source <- "herpes_2_hg52"
merged_peps_umap10 <- rbind(merged_peps_umap10, temp)
colnames(human_new_filt)[1] <- "Peptide"
temp <- merge(map10_peps, human_new_filt, by="Peptide")
temp$Source <- "human"
merged_peps_umap10 <- rbind(merged_peps_umap10, temp)
temp <- merge(map10_peps, random_new_filt, by="Peptide")
temp$Source <- "random"
merged_peps_umap10 <- rbind(merged_peps_umap10, temp)

write.table(merged_peps_umap10, "merged_peps_umap10mer.csv", row.names=FALSE, quote=FALSE, sep=',')
```

```{r merge files for 11mers}
map11_peps <- read.csv("11mer_pep_umap.csv", header=FALSE)
colnames(map11_peps) <- c("Peptide", "X", "Y")
merged_peps_umap11 <- merge(map11_peps, betaherpes_6a_new_filt, by="Peptide")
merged_peps_umap11$Source <- "betaherpes_6a"
temp <- merge(map11_peps, bk_new_filt, by="Peptide")
temp$Source <- "bk"
merged_peps_umap11 <- rbind(merged_peps_umap11, temp)
temp <- merge(map11_peps, cmv_new_filt, by="Peptide")
temp$Source <- "cmv"
merged_peps_umap11 <- rbind(merged_peps_umap11, temp)
temp <- merge(map11_peps, covid_new_filt, by="Peptide")
temp$Source <- "covid"
merged_peps_umap11 <- rbind(merged_peps_umap11, temp)
temp <- merge(map11_peps, ebv_new_filt, by="Peptide")
temp$Source <- "ebv"
merged_peps_umap11 <- rbind(merged_peps_umap11, temp)
temp <- merge(map11_peps, herpes_1_17_new_filt, by="Peptide")
temp$Source <- "herpes_1_17"
merged_peps_umap11 <- rbind(merged_peps_umap11, temp)
temp <- merge(map11_peps, herpes_2_hg52_new_filt, by="Peptide")
temp$Source <- "herpes_2_hg52"
merged_peps_umap11 <- rbind(merged_peps_umap11, temp)
colnames(human_new_filt)[1] <- "Peptide"
temp <- merge(map11_peps, human_new_filt, by="Peptide")
temp$Source <- "human"
merged_peps_umap11 <- rbind(merged_peps_umap11, temp)
temp <- merge(map11_peps, random_new_filt, by="Peptide")
temp$Source <- "random"
merged_peps_umap11 <- rbind(merged_peps_umap11, temp)

write.table(merged_peps_umap11, "merged_peps_umap11mer.csv", row.names=FALSE, quote=FALSE, sep=',')
```
...
rm(random_new_filt)
rm(human_new_filt)
rm(herpes_2_hg52_new_filt)
rm(herpes_1_17_new_filt)
rm(ebv_new_filt)
rm(covid_new_filt)
rm(cmv_new_filt)
rm(bk_new_filt)
rm(betaherpes_6a_new_filt)
```
...
netmhcpan_ranks <- read.csv("/Users/nguyenau/Documents/Coding/PeptideAlleleBinding/netmhcpan_ranks")
...
netmhcpan_ranks <- read.csv("/Users/nguyenau/Documents/Coding/PeptideAlleleBinding/netmhcpan_ranks")
head(hlathena_ranks)
mhcflurry_random <- read.csv("mhcflurry_random.csv")
mhcnuggets_random <- read.csv("random_mhcnuggets.txt", header=FALSE)
mhcflurry_random_order <- table(mhcflurry_random[mhcflurry_random$mhcflurry_prediction <= 500,]$allele)
mhcflurry_random_order <- as.data.frame(mhcflurry_random_order)

mhcnuggets_random_order <- as.data.frame(table(mhcnuggets_random[mhcnuggets_random$V2 <= 500,]$V3))
mhcnuggets_random_order$Var1 <- gsub(':', "", mhcnuggets_random_order$Var1)
mhcnuggets_random_order$Var1 <- gsub('_out.txt', "", mhcnuggets_random_order$Var1)
mhcnuggets_random_order$Var1 <- gsub('HLA-', "", mhcnuggets_random_order$Var1)

mhcnuggets_random_order$mhcnuggets_rank <- rank(-mhcnuggets_random_order$Freq)
mhcflurry_random_order$mhcflurry_rank <- rank(-mhcflurry_random_order$Freq)

combined_order <- merge(mhcflurry_random_order, mhcnuggets_random_order, by="Var1",all=TRUE)
cor.test(combined_order$mhcflurry_rank, combined_order$mhcnuggets_rank, method="spearman", use="pairwise.complete.obs")
combined_order <- merge(combined_order, hlathena_ranks[,c("Var1", "random")])

cor.test(combined_order$mhcflurry_rank, combined_order$random, method="spearman", use="pairwise.complete.obs")
cor.test(combined_order$random, combined_order$mhcnuggets_rank, method="spearman", use="pairwise.complete.obs")



mhcflurry_cmv <- read.csv("mhcflurry_cmv.csv")
mhcnuggets_cmv <- read.csv("cmv_mhcnuggets.txt", header=FALSE)
mhcflurry_cmv_order <- table(mhcflurry_cmv[mhcflurry_cmv$mhcflurry_prediction <= 500,]$allele)
mhcflurry_cmv_order <- as.data.frame(mhcflurry_cmv_order)

mhcnuggets_cmv_order <- as.data.frame(table(mhcnuggets_cmv[mhcnuggets_cmv$V2 <= 500,]$V3))
mhcnuggets_cmv_order$Var1 <- gsub(':', "", mhcnuggets_cmv_order$Var1)
mhcnuggets_cmv_order$Var1 <- gsub('_out.txt', "", mhcnuggets_cmv_order$Var1)
mhcnuggets_cmv_order$Var1 <- gsub('HLA-', "", mhcnuggets_cmv_order$Var1)

mhcnuggets_cmv_order$mhcnuggets_rank <- rank(-mhcnuggets_cmv_order$Freq)
mhcflurry_cmv_order$mhcflurry_rank <- rank(-mhcflurry_cmv_order$Freq)

mhcnuggets_merge <- merge(mhcnuggets_cmv_order, mhcnuggets_random_order, by="Var1")
cor.test(mhcnuggets_merge$mhcnuggets_rank.x, mhcnuggets_merge$mhcnuggets_rank.y, method="spearman",use="pairwise.complete.obs")

mhcflurry_merge <- merge(mhcflurry_cmv_order, mhcflurry_random_order, by="Var1")
cor.test(mhcflurry_merge$)
```
...
#library(GGally)
#names <- unique(merged_peps_umap8$Source)
#plot_list = list()
#count <- 1
...
```{r allplots}
temp8 <- unique(merged_peps_umap8[,2:3])
dim(temp)
ggplot(temp[sample(nrow(temp), 10000), ], aes(x=X, y=Y)) + geom_point()

temp9 <- unique(merged_peps_umap9[,2:3])
dim(temp9)
ggplot(temp9[sample(nrow(temp9), 10000), ], aes(x=X, y=Y)) + geom_point()

temp10 <- unique(merged_peps_umap10[,2:3])
dim(temp10)
ggplot(temp10[sample(nrow(temp10), 10000), ], aes(x=X, y=Y)) + geom_point()

temp11 <- unique(merged_peps_umap11[,2:3])
dim(temp11)
ggplot(temp11[sample(nrow(temp11), 10000), ], aes(x=X, y=Y)) + geom_point()
...
hlathena_seqs_8 <- list()
hlathena_seqs_9 <- list()
hlathena_seqs_10 <- list()
hlathena_seqs_11 <- list()
...
{
  test <- as.data.frame(hlathena_data[i])
  hlathena_seqs_8 <- append(hlathena_seqs_8, sum(nchar(test$sequence) == 8))
}

for (i in 1:length(hlathena_data))
{
  test <- as.data.frame(hlathena_data[i])
  hlathena_seqs_9 <- append(hlathena_seqs_9, sum(nchar(test$sequence) == 9))
}

for (i in 1:length(hlathena_data))
{
  test <- as.data.frame(hlathena_data[i])
  hlathena_seqs_10 <- append(hlathena_seqs_10, sum(nchar(test$sequence) == 10))
}

for (i in 1:length(hlathena_data))
{
  test <- as.data.frame(hlathena_data[i])
  hlathena_seqs_11 <- append(hlathena_seqs_11, sum(nchar(test$sequence) == 11))
}

hist(as.numeric(hlathena_seqs_8), xlim=range(0,3000))
hist(as.numeric(hlathena_seqs_9), xlim=range(0,3000))
hist(as.numeric(hlathena_seqs_10), xlim=range(0,3000))
hist(as.numeric(hlathena_seqs_11), xlim=range(0,3000))

table_lens <- table(nchar(hlathena_seqs))
table_lens


hlathena_training_alleles_order <- table(hlathena_seqs$V2)
hlathena_training_alleles_order <- as.data.frame(sort(hlathena_training_alleles_order))
hlathena_training_alleles_order
train_order <- merge(hlathena_ranks, hlathena_training_alleles_order, by="Var1")
cor.test(train_order$Freq, train_order$sd)
cor.test(train_order$Freq, train_order$avg)
...
for (i in 1:length(names))
{
  ...
  {
    x <- as.matrix(unique(merged_peps_umap8[merged_peps_umap8$Source == names[i],2:3]))
    ...
    y <- as.matrix(unique(merged_peps_umap8[merged_peps_umap8$Source == names[j],2:3]))
    #x <- matrix( rnorm(200), 100 , 2)
    bins <- bin2(y, ab, nbin)
    bin_sol <- bins$nc
    #sum(bin_sol)
...
library(reshape2)
matrix_bins8 <- as.data.frame(matrix_bins)
colnames(matrix_bins8) <- c("Var1", "Var2", "Diff")
matrix_bins8$Diff <- abs(matrix_bins8$Diff)
matrix_bins8$Var1 <- gsub("_scores", "", matrix_bins8$Var1)
matrix_bins8$Var2 <- gsub("_scores", "", matrix_bins8$Var2)

cormat <- cor(merged_order_8mers[,-1], method="spearman")
melted_cormat <- melt(cormat)
diffs <- merge(matrix_bins8, melted_cormat, by=c("Var1", "Var2"))
...
require(ash)
matrix_bins <- tibble(x=character(), y=character(), diff=numeric())
names <- unique(merged_peps_umap9$Source)
#names <- names[-8]
...
for (i in 1:length(names))
{
  for (j in (i+1):length(names))
  {
    x <- as.matrix(unique(merged_peps_umap9[merged_peps_umap9$Source == names[i],2:3]))
    #x <- matrix( rnorm(200), 100 , 2)
    ab <- matrix( c(-15,-15,15,15), 2, 2)      
    nbin <- c( 40, 40)                      # 1600 bins #compare binning to umap -> more bins(?)
    bins <- bin2(x, ab, nbin)
    ...
    y <- as.matrix(unique(merged_peps_umap9[merged_peps_umap9$Source == names[j],2:3]))
    #x <- matrix( rnorm(200), 100 , 2)
    bins <- bin2(y, ab, nbin)
    bin_sol <- bins$nc
    sum(bin_sol)
...
require(ash)
matrix_bins <- tibble(x=character(), y=character(), diff=numeric())
names <- unique(merged_peps_umap10$Source)
#names <- names[-8]
...
for (i in 1:length(names))
{
  for (j in (i+1):length(names))
  {
    x <- as.matrix(unique(merged_peps_umap10[merged_peps_umap10$Source == names[i],2:3]))
    #x <- matrix( rnorm(200), 100 , 2)
    ab <- matrix( c(-15,-15,15,15), 2, 2)      
    nbin <- c( 40, 40)                      # 1600 bins #compare binning to umap -> more bins(?)
    bins <- bin2(x, ab, nbin)
    ...
    y <- as.matrix(unique(merged_peps_umap10[merged_peps_umap10$Source == names[j],2:3]))
    #x <- matrix( rnorm(200), 100 , 2)
    bins <- bin2(y, ab, nbin)
    bin_sol <- bins$nc
    sum(bin_sol)
...
for (i in 1:length(names))
{
  for (j in (i+1):length(names))
  {
    x <- as.matrix(unique(merged_peps_umap11[merged_peps_umap11$Source == names[i],2:3]))
    #x <- matrix( rnorm(200), 100 , 2)
    ab <- matrix( c(-15,-15,15,15), 2, 2)      
    nbin <- c( 40, 40)                      # 1600 bins #compare binning to umap -> more bins(?)
    bins <- bin2(x, ab, nbin)
    ...
    y <- as.matrix(unique(merged_peps_umap11[merged_peps_umap11$Source == names[j],2:3]))
    #x <- matrix( rnorm(200), 100 , 2)
    bins <- bin2(y, ab, nbin)
    bin_sol <- bins$nc
    sum(bin_sol)
