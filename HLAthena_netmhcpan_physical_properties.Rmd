...
for (i in 1:length(names))
{
  for (j in (i+1):length(names))
  {
    ...
    bins <- bin2(as.matrix(unique(merged_peps_umap8[merged_peps_umap8$Source == names[i],2:3])), ab, nbin)
    bin_sol <- bins$nc
    #sum(bin_sol)
    #dim(x)[1]
    ...
    #x <- matrix( rnorm(200), 100 , 2)
    bins <- bin2(as.matrix(unique(merged_peps_umap8[merged_peps_umap8$Source == names[j],2:3])), ab, nbin)
    bin_sol <- bins$nc
    #sum(bin_sol)
    #dim(y)[1]
